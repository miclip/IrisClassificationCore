...
namespace Training
{
    class Program
    {
        static readonly string trainingDataFile = Path.Combine(Environment.CurrentDirectory, "Data", "iris-data.txt");
        static readonly string testDataFile = Path.Combine(Environment.CurrentDirectory, "Data", "iris-data-test.txt");
            ...
            TrainAsync(string trainingDataFile, string modelPath)
        {
            ...
            // The TextLoader loads a dataset with value columns and corresponding label column. 
            ...
            string testDatafile)
        {
            ...
            var testData = new TextLoader(testDatafile).CreateFrom<ClassificationData>(separator: ',');
